...
const dataPath = path.join(projectRoot, 'content', 'voice', 'iliad.json');
const data = JSON.parse(await fs.readFile(dataPath, 'utf8'));

if (!Array.isArray(data.books) || data.books.length !== 24) {
  throw new Error(`로컬 일리아스 데이터가 24권이 아닙니다: ${data.books?.length ?? 0}권`);
}
...
console.log(`[iliad-local] ${data.books.length}권, ${lineCount}행이 로컬 JSON에 저장되어 있습니다.`);
